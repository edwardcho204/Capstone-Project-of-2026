...
OUT_PATH = "all_station_lightgbm_model_results.csv"
...
for i, station_id in enumerate(stations, 1):
    print(f"\n{'='*80}")
    print(f"[{i}/{len(stations)}] Training LightGBM model for station {station_id}")
    print(f"{'='*80}")
...
results_df.to_csv("all_station_lightgbm_model_results.csv", index=False)
